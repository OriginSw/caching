...
namespace Sixeyed.Caching.AOP
{
    ...
    public static class Container
    {
        ...
        private static void Initialise()
        {
            ...
            //caches:
            RegisterSerializer(thisAssembly, Lifetime.Singleton);
            RegisterAllCache<ICache>(thisAssembly, Lifetime.Singleton);
        }

        public static void RegisterAllCache<T>(Assembly assembly, Lifetime lifetime) where T : ICache
        {
            RegisterAll<T>(assembly, lifetime, injectProperties: "Serializer");
        }

        public static void RegisterSerializer(Assembly assembly, Lifetime lifetime)
        {
            RegisterAll<Serializer>(assembly, lifetime, injectProperties: "Json");
        }
        ...
        public static void RegisterAll<T>(Assembly assembly, Lifetime lifetime, params string[] injectProperties)
        {
            Register<T>(assembly, lifetime, true, injectProperties);
        }
        ...
        private static void Register<T>(Assembly assembly, Lifetime lifetime, bool allowMultipleImplementations, params string[] injectProperties)
        {
            var injectionMembers = new List<InjectionMember>() { new InjectionConstructor() };
            foreach (var p in injectProperties)
            {
                injectionMembers.Add(new InjectionProperty(p));
            }
            ...
            foreach (var toType in toTypes)
            {
                ...
                if (allowMultipleImplementations)
                {
                    ...
                    {
                        _UnityContainer.RegisterType(actualFromType, toType, toType.Name, manager, injectionMembers.ToArray());
                        _UnityContainer.Configure<Microsoft.Practices.Unity.InterceptionExtension.Interception>()
                ...
                {
                    _UnityContainer.RegisterType(actualFromType, toType, manager, injectionMembers.ToArray());
                    _UnityContainer.Configure<Microsoft.Practices.Unity.InterceptionExtension.Interception>()
                        .SetInterceptorFor(toType, new VirtualMethodInterceptor());
                }
            }
